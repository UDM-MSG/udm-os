...
## 1) Purpose
Describe how **Auto Mechanics** maps into the UDM OS regulator. This annex explains the domain signals and how they compress into **S/C/P** -> **OPEN/WATCH/CLOSED** decisions with receipts and replay.

## 2) Signals -> S/C/P (bounded in [0,1])
- **Stability (S)**: engine/drive-train stability (RPM, temp), healthy sensor variance
- **Coherence (C)**: cross-sensor agreement (OBD-II), diagnostic code coherence, repetition consistency
- **Pressure (P)**: fault pressure (overheat, low oil pressure), misfire rates, abnormal vibrations
...
## 5) State semantics for Auto Mechanics
- OPEN: allow normal operation/assists
- WATCH: derate power, prompt service checks, extend monitoring
- CLOSED: safety shutdown, limp mode, hard block on risky maneuvers
