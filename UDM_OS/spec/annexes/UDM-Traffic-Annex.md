...
## 1) Purpose
Describe how **Traffic** maps into the UDM OS regulator. This annex explains the domain signals and how they compress into **S/C/P** -> **OPEN/WATCH/CLOSED** decisions with receipts and replay.

## 2) Signals -> S/C/P (bounded in [0,1])
- **Stability (S)**: flow stability (speed variance), route ETA stability, incident clearance consistency
- **Coherence (C)**: sensor/camera/app data agreement, route planner alignment, historical pattern fit
- **Pressure (P)**: congestion, incident density, queue growth, weather impact on mobility
...
## 5) State semantics for Traffic
- OPEN: enable optimized routing, green-wave coordination, normal dispatch
- WATCH: detour-prep, speed moderation, headway buffers, queue spillback guards
- CLOSED: block dangerous links, hold dispatch, force safest routes only
