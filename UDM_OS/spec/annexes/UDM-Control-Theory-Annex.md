...
**Intent:** Frame UDM in classical control terms (supervisory control and mode switching).

### Core Idea
- **Modes:** `CLOSED/WATCH/OPEN` = supervisory mode switching.
- **Damping / anti-windup analogue:** hysteresis depth `m` + symbolic threshold constraints.
- **Signals:** `S,C,P` as normalized error/quality/load measurements that gate actuation.
- **Corridors:** hard constraints on actuation (time/IO/network).

**Properties to state (symbolic):**
- Bounded switching frequency under noise for a given `m`.
- False-OPEN rate <= ε with adequate threshold separation.
