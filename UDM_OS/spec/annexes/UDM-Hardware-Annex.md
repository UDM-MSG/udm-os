...
**Intent:** Describe how to bind UDM semantics to hardware primitives (symbolically).

### Core Idea (symbolic bindings)
- Boot trust anchors and attestation (no keys, no vendor specifics).
- Isolation concepts (DMA fencing, process/VM isolation, privilege boundaries).
- Egress control concepts (line-rate or policy-enforced filtering).
