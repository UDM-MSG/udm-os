...
**Intent:** Map UDM to zero-trust, attestation, and runtime hardening.

### Core Idea
- `OPEN` only after attestation (symbolic description; no platform/vendor specifics).
- Receipts as signed decision artifacts (structure only, not keys).
- Corridors as enforcement zones (network, file, process).
- `WATCH` as quarantine or staged execution.
