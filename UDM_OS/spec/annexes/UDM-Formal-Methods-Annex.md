...
**Intent:** Define state transitions and invariants for verification (model checking / proofs).

### Core Idea
- Deterministic transition relation over `(state, signals, policy)` -> `(state', receipt?)`.
- **Invariants:** no actuation if state != `OPEN`; receipt integrity; replay equivalence.
- **Liveness:** if in-band holds for `m` consecutive windows, eventually `OPEN`.

**Proof artifacts (examples):**
- Safety: *No receipt -> No action*.
- Determinism: `/replay` outcome must match the receipt decision bits.
