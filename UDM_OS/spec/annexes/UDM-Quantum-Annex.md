...
**Intent:** Provide a state/observable/measurement vocabulary for cross-disciplinary rigor.

### Core Idea
- **State space:** `|CLOSED⟩, |WATCH⟩, |OPEN⟩` as a minimal orthogonal basis.
- **Observables:** `S, C, P ∈ [0,1]` (monotone with safety/consistency/load).
- **Update operator:** gates -> drivers -> hysteresis -> corridors.
- **Measurement:** the receipt; `/replay` is deterministic re-measurement.
- **Decoherence analogue:** hysteresis depth `m` damps flicker, enabling stable `OPEN`.
