...
## 1) Purpose
Describe how **Cosmology** maps into the UDM OS regulator. This annex explains the domain signals and how they compress into **S/C/P** -> **OPEN/WATCH/CLOSED** decisions with receipts and replay.

## 2) Signals -> S/C/P (bounded in [0,1])
- **Stability (S)**: time-series stability of observed signals (photometry, redshift bins), instrument baselines
- **Coherence (C)**: model/fit agreement (LambdaCDM or alternatives), cross-survey consistency
- **Pressure (P)**: anomaly/uncertainty pressure (outliers, instrument faults, calibration drift)
...
## 5) State semantics for Cosmology
- OPEN: permit normal data ingestion/pipeline steps
- WATCH: flag anomalies for review, increase calibration cadence
- CLOSED: pause ingestion, require re-calibration or quality-gate override
